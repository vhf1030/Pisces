...
from src.prediction.load_data import DataPreprocessor
...
class SeafoodPricePredictor:
    def __init__(self, model_paths, data_paths):
        ...
        :param model_paths: 어종별 사전 학습된 머신러닝 모델 경로 딕셔너리 (예: {"Mackerel": "path/to/mackerel_model.pkl"})
        :param data_paths: 어종별 데이터 파일 경로 딕셔너리 (예: {"Mackerel": "path/to/mackerel_data.csv"})
        """
...
if __name__ == "__main__":
    model_paths = {
        "Mackerel": "path/to/mackerel_model.pkl",
        "Tuna": "path/to/tuna_model.pkl",
        "Shrimp": "path/to/shrimp_model.pkl",
    }

    data_paths = {
        "Mackerel": "path/to/mackerel_data.csv",
        "Tuna": "path/to/tuna_data.csv",
        "Shrimp": "path/to/shrimp_data.csv",
    }

    predictor = SeafoodPricePredictor(model_paths, data_paths)

    # 특정 어종, 날짜, 시장에 대한 예측
    result = predictor.predict("2025-01-25", market="Market 1", seafood="Mackerel")
    print("예측 결과:", result)
